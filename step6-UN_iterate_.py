    ...
    try:
        ...
        with open(os.path.join(game_path, UNITY_WEBGL_BUILD_URL), 'r') as r:
            ...
            for key, value in build_json.items():
                if key not in ['companyName', 'productName', 'TOTAL_MEMORY', 'splashScreenStyle', 'backgroundColor'] and isinstance(value, str):
                    resource_urls.append(os.path.join(build_path, value))
                    if key not in ['dataUrl', 'asmCodeUrl', 'asmMemoryUrl', 'asmFrameworkUrl'] and isinstance(value, str):
                        print(key, value)
