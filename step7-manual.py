...
if not os.path.isfile('data/manual_URLs.txt'):
    exit('Please give me data/manual_URLs.txt')
...
with open('data/manual_URLs.txt', 'r') as r:
    for line in r.readlines():
        match = re.search(r'^https?://resource\.game\.nicovideo\.jp/games/gm([1-9][0-9]*)/[1-9][0-9]*/.+$', line.strip())
        if isinstance(match, re.Match):
            if int(match[1]) not in manual_games.keys():
                manual_games[int(match[1])] = []
            manual_games[int(match[1])].append(match[0])

games = list(filter(lambda item: item[0] in manual_games.keys(), games))
games.sort()
...
for game_id, key in games:
    # forced to activate timestamp
    timestamp_suffix = f'_{int(time.time()*1E6)}'
    try:
        if key is None:
        ...
        discovered_urls = []
        step_urls = manual_games[game_id]
